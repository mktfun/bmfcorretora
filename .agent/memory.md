...
- **Container Universal de Formulários**: Todos os wrappers de etapas (steps) e cards internos dos wizards DEVEM utilizar estritamente o Glassmorphism Escuro (`bg-white/5 border border-white/10 backdrop-blur-md`). É EXPRESSAMENTE PROIBIDO o uso de fundos pastéis/claros (`bg-muted`, `bg-slate`, `bg-gray`, `bg-white`).
- **Alertas e Caixas de Mensagem**: Cores semânticas (vermelho/amarelo) não devem usar fundos sólidos pastéis (ex: `bg-rose-50`). O padrão é fundo escuro com 10% de opacidade da cor e texto claro (ex: `bg-rose-500/10 border-rose-500/20 text-rose-200`).
- **Componentes de Grid para Opções Nativas (RadioCardGroup)**: O estado inativo usa Pills em Glassmorphism (`bg-white/5 text-white/70`). O estado ativo deve **obrigatoriamente usar Laranja** (`bg-[#e8702a]/20 border-[#e8702a] text-white shadow-[0_0_15px_rgba(232,112,42,0.2)]`). Bolinhas nativas de radio/check devem ser removidas para manter interface limpa.
- **Campos de Estado Civil (`MaritalStatusGrid`)**: Devem **obrigatoriamente** usar uma Lista Suspensa (Select do Shadcn) ao invés de Pills, para otimizar espaço e seguir a preferência explícita do usuário. As bordas no `focus` devem ser Laranja (`focus:border-[#e8702a]`).
- **Animações e Tipografia (Cards Principais)**: Evitar animações iterativas de entrada (como `whileInView` do framer-motion variando opacidade) que causem flashes/piscadas visuais ao rolar a página. Os títulos dos Cards de Seguro devem adotar fonte Sans-serif padrão (Inter) e NUNCA `font-playfair` ou `italic` para manter legibilidade moderna. Isso vale para TODOS os seguros, incluindo Vida e Consórcios.
- **Arquitetura de Landing Page (Padrão SaaS B2B)**: As páginas informativas devem seguir o design pattern Altium/Enterprise. Elementos exigidos: **Hero** dividida (esq: Copy, dir: Sticky Form/CTA), sessão **Comparativo** (Old Way burocrático vs New Way digital), **Workflow Interconectado** (Cards com setas simulando "Cote -> Analise -> Proteja") e **Carrossel de Depoimentos** com rolagem horizontal livre. Animações via Framer Motion focadas em Slide-Up suave.
- **Contraste e Legibilidade (Wizards)**: Nunca utilize a classe `text-foreground` para textos, labels ou subtítulos dentro dos formulários (Wizards) de tema escuro, pois em determinados contextos ela renderiza azul marinho escuro. Use sempre explicitamente `text-white` ou `text-white/80` para assegurar contraste absoluto. O componente `Label.tsx` base deve conter `text-white` nativamente.
- **Títulos e Legendas de Wizards (Proibição Estrita)**: NUNCA crie ou injete headers customizados, legendas ou ícones adicionais (seja em laranja ou outra cor) diretamente dentro dos componentes Wizard (ex: `AutoWizard.tsx`, `SinistroWizard.tsx`, etc.). Os títulos oficiais já são renderizados pela página raiz (`Cotacao.tsx`, etc.). Injetar um novo título resultará em duplicação, poluindo a UI. O primeiro elemento visual interno de um Wizard deve ser sempre o componente `<Stepper>`.

## Integração Aceternity & Animações (Novos Aprendizados)
- **Componentização Estrita**: Efeitos complexos baseados no Aceternity (como Hero Spotlights, Moving Borders, Floating Particles e Bento Grids) devem ser construídos como componentes isolados (ex: `BentoFeatures.tsx`, `SaaSHero.tsx`) e injetados na raiz da `Index.tsx`. Isso evita poluir as regras de roteamento global.
- **Animações Aceternity / Hover**: Evite adicionar regras de CSS/Tailwind que desloquem o elemento do eixo físico (como `hover:-translate-y-1` ou alterações de `margin`/`padding`) em conjunto com grids super reativos ou efeitos radiantes sob o cursor. Durante o scroll da página, isso gera um loop infinito (flickering), já que o item entra em estado hover sob o ponteiro inerte, altera seu tamanho, sai debaixo do mouse, e "pisca". Privilegie transições puras visuais (cor de fundo, glow de borda, opacidade).
- **Carrosséis Infinitos (Marquee)**: Ao invés de usar `snap-x` nativo com botões de scroll lateral (que são limitados em responsividade e UX), a preferência para depoimentos e logos é usar um *Infinite Marquee* contínuo. A implementação deve duplicar o array e usar Framer Motion: `animate={{ x: ["0%", "-50%"] }}` com transição `linear` e `repeat: Infinity`.
- **Efeitos de Hover no Glassmorphism**: Cards de Bento Grid devem incluir uma camada base de cor invisível que transiciona a opacidade no hover (ex: `bg-gradient-to-br from-cyan-500/0 to-cyan-500/5 opacity-0 group-hover:opacity-100 pointer-events-none`) para gerar um brilho interno premium sem perder a essência obscura do Dark Mode BMF.
