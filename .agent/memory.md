...
- **Animações e Tipografia (Cards Principais)**: Evitar animações iterativas de entrada (como `whileInView` do framer-motion variando opacidade) que causem flashes/piscadas visuais ao rolar a página. Os títulos dos Cards de Seguro devem adotar fonte Sans-serif padrão e NUNCA `font-playfair` ou `italic` para manter legibilidade moderna.
