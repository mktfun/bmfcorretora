...
import { ArrowRight, Building, Car, Briefcase, TrendingUp, BadgePercent, Shield } from "lucide-react";

const features = [
  {
    icon: Building,
    title: "Imóveis",
    description: "Planeje a compra da casa própria, sala comercial ou construção com parcelas que cabem no bolso.",
  },
  {
    icon: Car,
    title: "Veículos",
    description: "Troque de carro ou compre seu primeiro veículo 0km ou seminovo de forma programada.",
  },
  {
    icon: Briefcase,
    title: "Serviços",
    description: "Pague reformas, festas, viagens ou procedimentos estéticos sem se descapitalizar.",
  },
];

const benefits = [
  { icon: TrendingUp, text: "Sem juros — só taxa de administração" },
  { icon: BadgePercent, text: "Parcelas até 50% menores que financiamento" },
  { icon: Shield, text: "Garantia de crédito assegurada" },
];
    ...
    <div className="min-h-screen flex flex-col bg-black">
      ...
      <main className="relative z-10 flex-1 pt-32 pb-24">
        <div className="container mx-auto px-4 md:px-6">
          ...
          >
            {benefits.map((b, i) => (
              <div key={i} className="flex items-center gap-2 px-4 py-2 bg-white/5 border border-white/10 rounded-full text-sm text-neutral-300">
                <b.icon size={14} className="text-cyan-400 flex-shrink-0" />
                {b.text}
              </div>
            ))}
          </motion.div>
